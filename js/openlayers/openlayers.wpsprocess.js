...
OpenLayers.WPSProcess = OpenLayers.Class({
    ...
    execute: function(options) {
        this.configure({
            ...
            callback: function() {
                ...
                (function callback() {
                	if(me.executeCallbacks){OpenLayers.Util.removeItem(me.executeCallbacks, callback);}
                    ...
                    /*******************************************************************************************************************************************************
                     * ADDED if description.dataInputs is optional (minOccurs=0) and data.Inputs.data.IteralData.value==undefined, then delete this item in the dataInputs
                     */
                    var dataInputs=me.description.dataInputs, data;
                    for(var d in dataInputs){
                    	data=dataInputs[d];
                    	if(data.minOccurs==0 && data.data && data.data.literalData && (data.data.literalData.value==undefined || data.data.literalData.value=="")){
                    		dataInputs[d]=null;
                    	}
                    }
                    //*******************************************************************************************************************************************************
                    ...
                    OpenLayers.Request.POST({
                        ...
                        success: function(response) {
                            ...
                            var mimeType = (output.complexOutput) ? me.findMimeType(output.complexOutput.supported.formats) : null;
                            ...
                            try{
                            	...
                            	console.log("[ERROR] WPSProcess.execute.callback success: " + e.message);
                            	console.log("[ERROR] WPSProcess.execute.callback success: " + response.responseText);
                            	console.log("[ERROR] WPSProcess.execute.callback success POST data:");	
                            	console.log(new OpenLayers.Format.WPSExecute().write(me.description));
                            }
                        },
                        scope: me
                    });
    ...
    setInputData: function(input, data) {
        ...
        if (data instanceof OpenLayers.WPSProcess.ChainLink) {
        	...
        	/*******************************************************************************************************************************************************
             * ADDED if data is an instance of WPSProcess.Reference
            */
        	if(data instanceof OpenLayers.WPSProcess.Reference){
	            ...
	            if (complexData) {
					...
					/**************************************************************************************************************************
					 * if format=application/json,  clear the attributes of features to avoid execution error
					 */
					if(format=="application/json"){
						var features=this.toFeatures(data),
							cloneF;
						this.cloneFeatures=[];
						for(var f in features ){
							cloneF=features[f].clone();
							cloneF.attributes={};
							this.cloneFeatures.push(cloneF);
						}
					}
					//**************************************************************************************************************************
	                ...
	                input.data.complexData = {
	                    mimeType: format,
	                    value: (function(){
	                    	if(format == "application/wkt" || format=="application/json"){
	                    		return me.formats[format].write(me.cloneFeatures);
	                    	}else{
	                    		return data;
	                    	}
	                    })()
	                ...
	                /**************************************************************************************************************************
					 * if format=application/json then delete properties in the json, because it will make error for geoserver to parse WPS inputs
					 */
	                if(format=='application/json'){
	                	input.data.complexData.value=input.data.complexData.value.replace(/\"properties":{},/g, "");
	                }
	                //**************************************************************************************************************************
	            } else {
    ...
    setResponseForm: function(options) {
        ...
        this.description.responseForm = {
            rawDataOutput: {
                ...
                mimeType: (output.complexOutput) ? this.findMimeType(output.complexOutput.supported.formats, options.supportedFormats) : null
            }
        };
    },
...
/******************************************************************************************************************************************************************
 * Class: OpenLayers.WPSProcess.Reference
 * reference to WFS or other resource
 */
OpenLayers.WPSProcess.Reference = OpenLayers.Class({
    ...
    CLASS_NAME: "OpenLayers.WPSProcess.Reference"
